...
with open(args.trace, "r") as f:
        ...
        while line != "":
            if line.startswith(" qemu-system-x86"):
                ...
                if event == "kvm_hypercall":
                        nr = get_part(parts, "nr")
                        a0 = get_part(parts, "a0")
                        a1 = get_part(parts, "a1")
                        a2 = get_part(parts, "a2")
                        a3 = get_part(parts, "a3")
                        hypercall_data.append((timestamp, second, nr, a0, a1, a2, a3))
...
df = pd.DataFrame.from_records(hypercall_data, columns=["timestamp_raw", "second", "nr", "a0", "a1", "a2", "a3"])
df["timestamp"] = pd.to_datetime(df["timestamp_raw"], unit='s')
df["minute"] = df["second"] // 60

print(df.groupby(["nr"]).count())
print(df.groupby(["a0"]).count())
print(df.groupby(["a1"]).count())
print(df.groupby(["a2"]).count())
print(df.groupby(["a3"]).count())